...
interface Paper {
  ...
  title: string
  author: string
  authorEmail: string // Added author email field
  coAuthors?: string[] // Added co-authors field
  theme: string
...
export default function PapersPage() {
    ...
    const fetchPapers = async () => {
      ...
      try {
        ...
        setPapers(result.data.map((p: any) => ({
          ...
          authorEmail: p.author?.email || '', // Map author email
          coAuthors: p.coAuthors || [], // Map co-authors
          theme: p.theme?.name || '',
          status: p.status,
          fileUrl: p.fileUrl,
  ...
  const updatePaperStatus = async (id: string, newStatus: Paper['status']) => {
    try {
      // Find the paper before updating
      const paperToUpdate = papers.find(paper => paper.id === id);
      ...
      setPapers(prev =>
        prev.map(paper =>
          paper.id === id ? { ...paper, status: paper.status } : paper
        )
      )
    }
  }
